...
- **体验地址**: [neon-vibe-motion.vercel.app](https://neon-vibe-motion.vercel.app)
- **更多 Demos**: [neon-vibe-motion.vercel.app/#/demos](https://neon-vibe-motion.vercel.app/#/demos)

> **关于在线版**: 为绕过浏览器 CORS 限制，API 请求会经由 Vercel Serverless Function 转发，**超时上限为 300 秒**。你的 API Key 仅在服务端转发时使用，不会被存储。如需更长时间的生成或不受限制的使用体验，建议通过 `pnpm dev` 本地部署。中国大陆用户访问在线版可能需要一点"魔法"。
