...
import { extractMotion, validateMotion, type NeonFile } from '../schema.js';
...
export async function renderCommand(inputPath: string, options: RenderOptions): Promise<void> {
  ...
  try {
    ...
    if (options.width) renderOptions.width = options.width;
    if (options.height) renderOptions.height = options.height;
    if (options.alpha) renderOptions.alpha = options.alpha;
    if (options.resolution) renderOptions.resolution = options.resolution;
    if (options.aspectRatio) renderOptions.aspectRatio = options.aspectRatio;
    if (options.paramOverrides) renderOptions.paramOverrides = options.paramOverrides;

    // Set up progress listener in browser context
    await page.evaluate(() => {
      window.addEventListener('message', (e) => {
        if (e.data?.type === 'neon:progress') {
          (window as any).__lastProgress = e.data.progress;
        }
      });
    });
    ...
    const progressInterval = setInterval(async () => {
      try {
        const progress = await page.evaluate(() => (window as any).__lastProgress);
    ...
    const result = await page.evaluate(
      async ({ motion, opts }) => {
        const renderResult = await (window as any).NeonHarness.render(motion, opts);
